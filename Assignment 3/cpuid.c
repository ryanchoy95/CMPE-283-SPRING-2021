...
int kvm_emulate_cpuid(struct kvm_vcpu *vcpu)
{
	...
	if(eax == 0x4FFFFFFF){
		u64 theTime = 0;
		theTime = atomic_long_read(&time);
		eax = atomic_read(&exit_counter);
		ebx = theTime >>32 & 0xFFFFFFFF;
		ecx = theTime & 0xFFFFFFFF;
	}
		...
		for(i = 0; i<70;i++){
			if(ecx == i){
				...
				else if(ecx == 3 && ecx == 4 && ecx == 11 && ecx == 16 && ecx == 17 && ecx == 30 && ecx == 33 && ecx == 34 && ecx == 51 && ecx == 63 && ecx == 64 && ecx == 66 && ecx == 67 && ecx == 68 && ecx == 69)  
				{
					...
					eax = atomic_read(&number_of_exits[i]);
					ebx = 0;
					ecx = 0;
					edx = 0;
